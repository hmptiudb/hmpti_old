...
class P extends CI_Controller {
	...
	public function index()
	{
		...
		$data['daftarAk'] = $this->Model_detail_organisasi->get()->row_array();
...
// echo 'LOHHHH KOK ERROR';
	...
	public function divisi($id_divisi)
	{
		...
		$data['status'] = $this->Model_statusProker->read()->result_array();
		$data['findstatus'] = $this->Model_statusProker->find($data["proker_jamak"][0]["id_status"])->result_array();
		foreach($data["proker_jamak"] as $key => $value){
			$data["proker_jamak"][$key]["id_status"] = $this->Model_statusProker->find($data["proker_jamak"][$key]["id_status"])->result_array()[0];
		}
		...
		public function lpj()
	{
		...
		// echo "<pre>"; var_dump( $data ); die();
		$this->load->view('end_user/templates/header', $data);
		$this->load->view('end_user/lpj', $data);
		$this->load->view('end_user/templates/footer', $data);
	}
	...
	*/
    	...
    	public function downloadFile()
	{
        ...
        force_download('assets/pdf/FP1_HMPTI_2022.docx', NULL);
        // redirect( base_url() );
	}
    ...
    public function formtambah()
  {
    if ($this->input->is_ajax_request() == true) {
      $msg = [
        'success' => $this->load->view('end_user/oprek/form_tambah', '', true)
      ];
...
//                 {
//                 	$this->session->set_flashdata("msg", "error#Pendaftaran Anda gagal pastikan semua form terisi");
//                 //  $error = array('error' => $this->upload->display_errors());
            
//                 $this->session->set_flashdata("msg", "error#Data gaagal diUpload.");
//                 // redirect(base_url() . "admin/divisi",$error);
//     	           redirect('/', 'refresh');
...
//                     $this->upload->do_upload('formulir_pen');
...
//                     if ( ! $this->upload->do_upload('formulir_pen') &&  ! $this->upload->do_upload('cv') && ! $this->upload->do_upload('krs')   )
//                     {
//                             $error = array('error' => $this->upload->display_errors());
//                             // $this->session->set_flashdata("msg", "error#FILE HARUS SESUAI");
//                             // redirect(base_url('/') ,$error);
//                             $msg = [
//                                 'error' =>  $this->load->view('end_user/oprek/form_tambah', '', true)
//                               ];
//                               echo json_encode($msg);
...
//                             $this->Model_oprek->add($data);
                          
//                             $msg = [
//                                 'sukses' => $this->load->view('home/modaledit', $data, true)
//                               ];
//                               echo json_encode($msg);
                              
//                     // 		$this->session->set_flashdata("msg", "success#Anda Berhasil mendaftar");
          ...
          $noHp = $this->input->post('noHp', true);
                   ...
                   $nama = time();
                    ...
                    $file3 = $this->upload->data();
                     ...
                     $this->form_validation->set_rules('nim', 'Nim', 'trim|required|is_unique[h_oprekk.nim]', [
                        'required' => '%s tidak boleh kosong',
                        'is_unique' => '%s sudah ada dalam database',
                      ]);
                      
                    
                    if ($this->form_validation->run() == true &&  $this->upload->do_upload('formulir_pen') &&  $this->upload->do_upload('cv') && $this->upload->do_upload('krs')  ) {
