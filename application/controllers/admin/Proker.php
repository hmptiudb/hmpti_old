...
class Proker extends CI_Controller
{
	...
	public function i($id_divisi='')
	{
		...
		$data['findstatus'] = $this->Model_statusProker->find($data["main_data"][0]["id_status"])->result_array();
		foreach($data["main_data"] as $key => $value){
			$data["main_data"][$key]["id_status"] = $this->Model_statusProker->find($data["main_data"][$key]["id_status"])->result_array()[0];
		}
