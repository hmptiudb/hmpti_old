...
<script>
  ...
  function edit_prepare(id) {
    ...
    let isistatus = $("#status-"+id).text();
    let select = selectStatus(isistatus);
    ...
    $("#edit_status").html(select);
    
  }
  function selectStatus(status){
    let arrstatus = <?= json_encode($status); ?>;
    let select = []

    arrstatus.forEach(function(obj) { 
      select.push(`<option value="${obj.id_status}"${status == obj.text ? ' selected' : ''}>${obj.text}</option>`)
    });

    return select;
  }

  function statustoadd(){
    let isistatus = ''
    let select = selectStatus(isistatus);
    $("#add_status").html(select);
  }
